...
namespace fion
{
    ...
    const std::string Response::toRawResponse(void) const
    {
        std::ostringstream rawResponse;

        // Start line
        rawResponse << HTTPVersionToString(_HTTPVersion) << " " << static_cast<unsigned short>(_statusCode) << " " << statusCodeToString(_statusCode) << "\r\n";
